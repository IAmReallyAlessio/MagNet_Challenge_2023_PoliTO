...
def core_loss(B, F, T):
    ...
    if not (MATERIAL == 'Material D'):
        model_trapz_flat = keras.models.load_model(f".\\Models\\{MATERIAL} trapz flat.keras")
        norm_func_trapz_flat = pd.read_csv(f".\\Models\\{MATERIAL} info trapz flat.csv")
    ...
    B_waveform = pd.DataFrame(B)
    Frequency = pd.DataFrame(F)
    Temperature = pd.DataFrame(T)
    ...
    #model_tri.summary()
    output_energy = model_tri.predict(input_db)     #Model outputs the Energy (Power_Loss / Frequency)
    ...
    for num in indices_trapz:
        ...
        df_trapz.loc[num, 'flat_time'] = len(flat_elem) * dt / Per
        df_trapz.loc[num, 'dB'] = aux_input
        #print(f_eq_orig)
        flat_elem = list(filter(lambda x: x < 5e3, f_eq_orig))
        df_trapz.loc[num, 'flat_time'] = len(flat_elem)*dt/Per
        df_trapz.loc[num, 'dB'] = aux_input
        if len(flat_elem) > 5 and not(MATERIAL=='Material D') :
                idx_flat.append(num)
        else: #for other trapezioidal waves use composition method
    ...
    df_trapz_flat = df_trapz.loc[idx_flat]
    idx_oth = list(set(indices_trapz) - set(idx_flat))
    df_trapz_oth = df_trapz.loc[idx_oth]

    #print(df_trapz_oth.to_string())

    #%% For the flat trapezioidal use a specific neural network
    if not(MATERIAL=='Material D'):    
        input_db = pd.DataFrame()
        #print(norm_func_trapz_flat)
        input_db['F'] = ( np.log10(df_trapz_flat['Frequency'].to_numpy()) - norm_func_trapz_flat['Freq'].iloc[0] ) / norm_func_trapz_flat['Freq'].iloc[1] 
        input_db['PkPk'] = ( np.log10(df_trapz_flat['B_pkpk'].to_numpy()*1e3) - norm_func_trapz_flat['PkPk'].iloc[0] ) / norm_func_trapz_flat['PkPk'].iloc[1] 
        input_db['T'] = ( df_trapz_flat['Temperature'].to_numpy() - norm_func_trapz_flat['Temp'].iloc[0] ) / norm_func_trapz_flat['Temp'].iloc[1] 
        input_db['dB'] = ( df_trapz_flat['dB'].to_numpy() - norm_func_trapz_flat['dB'].iloc[0] ) / norm_func_trapz_flat['dB'].iloc[1] 
        #print(input_db.to_string())

        output_trapz_flat = model_trapz_flat.predict(input_db, verbose=0)
        #print(output_trapz_flat)
        df_trapz_flat['Power_Loss'] = 10**output_trapz_flat.flatten()*df_trapz_flat['Frequency'].values/1e3
    #print(df_trapz_flat.to_string())
    #%% Assembly togheter all dataframes
    final_df = pd.concat([pd.DataFrame(), df_tri])
    final_df = pd.concat([final_df, df_sin])
    final_df = pd.concat([final_df, df_trapz_oth])
    final_df = pd.concat([final_df, df_trapz_flat])

...
def set_input(freq, flux, duty, temp):
    ...
    temp = np.concatenate((Freq_t,Flux_t,Duty_t, Temperature_t),axis=1)

    in_tensors = torch.from_numpy(temp).view(-1, 4)
    out_tensors = torch.from_numpy(Temperature_t).view(-1, 1)
...
def main():
    ...
    modello.load_state_dict(torch.load("/content/Model_FNN_N87.sd"))
    ...
    testing = set_input(np.array([354610,350000]), np.array([0.05669549996007316,0.05669549996007316]), np.array([0.6982421875,0.7]), np.array([90,75]))
    ...
    with torch.no_grad():
        for inputs, labels in test_loader:
            y_pred.append(modello(inputs.to(device)))
    
    
    y_pred = torch.cat(y_pred, dim=0)
 
    risultato = 10**(y_pred.cpu().numpy())
    print(risultato)
